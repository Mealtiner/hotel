...
if ( is_array( $grid_visibility ) && array_key_exists( 'garry-kategorie-pokoju', $grid_visibility ) ) {
	unset( $grid_visibility['garry-kategorie-pokoju'] );
...
foreach ( wp_roles()->roles as $role_key => $role_info ) {
	...
	if ( $role && $role->has_cap( 'garry_grid_manage_kategorie_pokoju' ) ) {
		$role->remove_cap( 'garry_grid_manage_kategorie_pokoju' );
	}
}
foreach ( get_users( array( 'fields' => array( 'ID' ) ) ) as $u ) {
	$user = get_user_by( 'id', $u->ID );
	if ( $user && $user->has_cap( 'garry_grid_manage_kategorie_pokoju' ) ) {
		$user->remove_cap( 'garry_grid_manage_kategorie_pokoju' );
	}
}
