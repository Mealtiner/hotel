<?php
/**
 * Plugin Name:       GARRY – Hero křivka trati
 * Plugin URI:        https://www.garry.cz
 * Description:       Animovaná červená „křivka trati" na pozadí úvodní hero sekce GRID Hotelu, která se po načtení stránky postupně vykreslí zleva doprava (simulace průjezdu). V nastavení lze měnit tloušťku křivky, rychlost průjezdu a zapnout/vypnout zobrazení. Součást ekosystému GARRY Promotion.
 * Version:           1.1.0
...
function garry_hk_defaults() {
	return array( 'enabled' => 1, 'thickness' => 3, 'speed' => 2.6 );
}
...
Garry_Promotion_Registry::register( array(
	'slug' => 'garry-hero-krivka', 'title' => 'Hero křivka trati',
	'callback' => 'garry_hk_admin_page', 'plugin_file' => __FILE__,
	'doc' => '<p>Křivka Masarykova okruhu v hero sekci titulní stránky se vykresluje automaticky — bez shortcodu. Nastavení pouze zde (administrátor).</p>', 'dashicon' => 'dashicons-flag', 'position' => 15,
) );
...
function garry_hk_admin_page() {
	  ...
	  <div style="flex:0 0 360px">
	    ...
	    <div style="background:#0d0f12;border:1px solid #ccd0d4;border-radius:8px;overflow:hidden">
	      <svg id="hk-svg" viewBox="0 0 1000 560" preserveAspectRatio="none" style="width:100%;height:230px;display:block">
	        <path id="hk-path" d="M-20 420 C 200 380, 240 250, 430 250 S 720 340, 820 250 S 900 120, 1040 160" fill="none" stroke="#C20E1A" stroke-width="3" stroke-linecap="round"/>
	      </svg>
	    </div>
	...
	(function(){
	  ...
	  var path=document.getElementById('hk-path'), en=q('enabled'), th=q('thickness'), sp=q('speed');
	  ...
	  [en,th,sp].forEach(function(el){ el.addEventListener('input',play); el.addEventListener('change',play); });
...
/* Frontend: konfiguraci předáme child theme skriptu (grid.js) přes globální objekt.
 * Musí být v <head>, tj. dřív než footer grid.js. */
