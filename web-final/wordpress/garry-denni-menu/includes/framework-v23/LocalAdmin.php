...
final class LocalAdmin {
	 ...
	 */
	/**
	 * Verze s víc provozy (viz garry-denni-menu.php, garry_menu_own_tabs()):
	 * místo jednoho pevného "own" tabu (klíč '') vykreslí N dynamických tabů
	 * podle $config['own_tabs'] (slug provozu => název, + 'settings') jako
	 * PŘÍMÉ sourozence Přehledu/Info/Logu ve stejném řádku – přesně jak bylo
	 * v zadání ("před ně by se přidávaly karty podle provozu"). Beze změny
	 * zůstává, jak se vykresluje Přehled/Info/Log samotné (sdílené chování
	 * napříč pluginy) – mění se jen "own" část a routing k ní.
	 */
	public static function render( array $config ) {
		...
		$own_tabs = isset( $config['own_tabs'] ) && is_array( $config['own_tabs'] ) ? $config['own_tabs'] : array();

		$tab          = isset( $_GET['garry_v23_tab'] ) ? sanitize_key( wp_unslash( $_GET['garry_v23_tab'] ) ) : '';
		$allowed_tabs = array_merge( array_keys( $own_tabs ), array( '', 'overview', 'info', 'log' ) );
		if ( ! in_array( $tab, $allowed_tabs, true ) ) {
			$tab = '';
		}
		if ( '' === $tab && ! empty( $own_tabs ) ) {
			$keys = array_keys( $own_tabs );
			$tab  = $keys[0];
		}

		echo '<div class="wrap garry-v23-wrap">';
		echo '<h1>' . esc_html( $config['display_name'] ) . '</h1>';
		self::render_tabs( $config, $tab, $own_tabs );
		...
		switch ( $tab ) {
			...
			case 'log':
				...
				break;
			default:
				if ( isset( $own_tabs[ $tab ] ) && ! empty( $config['own_callback'] ) && is_callable( $config['own_callback'] ) ) {
					call_user_func( $config['own_callback'], $tab );
				} elseif ( ! empty( $config['own_callback'] ) && is_callable( $config['own_callback'] ) ) {
	...
	private static function render_tabs( array $config, $current, array $own_tabs = array() ) {
		$base = admin_url( 'admin.php?page=' . $config['menu_slug'] );
		$tabs = array();
		if ( ! empty( $own_tabs ) ) {
			$tabs = $own_tabs;
		} elseif ( ! empty( $config['own_callback'] ) ) {
